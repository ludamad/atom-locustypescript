...
//                    prev = bodyScan.merge(prev);
        ...
        function getBrandInterfaceRefactorData(node: Node): RefactorData {
            ...
            if (locusTypeDecl) {
                var {pos, end} = locusTypeDecl;
                var replaceSpan = createTextSpanFromBounds(pos, end);
            } else {
                var {pos} = getModuleOrSourceFileOrFunction(node);
                var replaceSpan = createTextSpanFromBounds(pos, pos);
            }
            let replaceText = getBrandInterfaceRefactorTextFromLocusType(targetType);
            return {replaceSpan, replaceText};
            // Where:
            function getBrandInterfaceRefactorTextFromLocusType(type: LocusType): string {
                ...
                let members = getPropertiesOfType(type).map(symbolToMember);
                let startingSpaces = getSourceIndentPrefix(getSourceFileOfNode(node), pos);
                let lines = [`\nbrand interface ${typeToString(targetType)}${extendsPart} {\n`];
                for (let member of members) {
                    lines.push(`    ${member}\n`);
                }
                lines.push("}\n");
                return lines.map(x => startingSpaces + x).join('');
            }
