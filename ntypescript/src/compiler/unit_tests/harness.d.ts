/// <reference path="../../server/protocol.d.ts" />
/// <reference path="../../server/node.d.ts" />
/// <reference path="../../harness/external/mocha.d.ts" />
/// <reference path="../../harness/external/chai.d.ts" />
...
declare namespace ts {
    ...
    interface LocusTypeExtractionInfo {
        content: string;
...
declare namespace ts.server {
    ...
    class ProjectService {
        ...
        removeConfiguredProject(project: Project): void;
         ...
         */
        openFile(fileName: string, openedByClient: boolean): ScriptInfo;
        findConfigFile(searchPath: string): string;
        /**
         * Open file whose contents is managed by the client
         * @param filename is absolute pathname
         */
        openClientFile(fileName: string): ScriptInfo;
...
declare namespace ts.server {
    ...
    class Session {
        ...
        private errorCheck(file, project);
        ...
        private openClientFile(fileName);
...
declare namespace ts.server {
    ...
    class SessionClient implements LanguageService {
        ...
        private lineOffsetToPosition(fileName, lineOffset);
...
declare module Harness.SourceMapRecoder {
    function getSourceMapRecord(sourceMapDataList: ts.SourceMapData[], program: ts.Program, jsFiles: Compiler.GeneratedFile[]): string;
}
...
declare module Utils {
    ...
    function memoize<T extends Function>(f: T): T;
    function assertInvariants(node: ts.Node, parent: ts.Node): void;
    ...
    function assertDiagnosticsEquals(array1: ts.Diagnostic[], array2: ts.Diagnostic[]): void;
    function assertStructuralEquals(node1: ts.Node, node2: ts.Node): void;
}
declare module Harness.Path {
...
declare module Harness {
    ...
    module Compiler {
         ...
         */
        class WriterAggregator implements ITextWriter {
            lines: string[];
            currentLine: string;
        ...
        interface IEmitterIOHost {
            writeFile(path: string, contents: string, writeByteOrderMark: boolean): void;
            resolvePath(path: string): string;
        }
        /** Mimics having multiple files, later concatenated to a single file. */
        class EmitterIOHost implements IEmitterIOHost {
            private fileCollection;
            /** create file gets the whole path to create, so this works as expected with the --out parameter */
            writeFile(s: string, contents: string, writeByteOrderMark: boolean): void;
        ...
        let defaultLibFileName: string;
        let defaultLibSourceFile: ts.SourceFile;
        let defaultES6LibSourceFile: ts.SourceFile;
        let fourslashFileName: string;
        let fourslashSourceFile: ts.SourceFile;
        function getCanonicalFileName(fileName: string): string;
        function createCompilerHost(inputFiles: {
            unitName: string;
            content: string;
        }[], writeFile: (fn: string, contents: string, writeByteOrderMark: boolean) => void, scriptTarget: ts.ScriptTarget, useCaseSensitiveFileNames: boolean, currentDirectory?: string, newLineKind?: ts.NewLineKind): ts.CompilerHost;
        interface HarnessOptions {
            useCaseSensitiveFileNames?: boolean;
            includeBuiltFile?: string;
            baselineFile?: string;
        }
        function setCompilerOptionsFromHarnessSetting(settings: Harness.TestCaseParser.CompilerSettings, options: ts.CompilerOptions & HarnessOptions): void;
        class HarnessCompiler {
            ...
            private lastErrors;
            reset(): void;
            reportCompilationErrors(): ts.Diagnostic[];
            setCompilerSettings(tcSettings: Harness.TestCaseParser.CompilerSettings): void;
            addInputFiles(files: {
                unitName: string;
                content: string;
            }[]): void;
            ...
            emitAll(ioHost?: IEmitterIOHost): void;
            createProgram(inputFiles: {
                unitName: string;
                content: string;
            }[], otherFiles: {
                unitName: string;
                content: string;
            }[], onComplete: (result: CompilerResult, program: ts.Program) => void, settingsCallback?: (settings: ts.CompilerOptions) => void, options?: ts.CompilerOptions & HarnessOptions, currentDirectory?: string): {
                program: ts.Program;
                emit: () => GeneratedFile[];
            };
            compileFiles(inputFiles: {
                unitName: string;
                content: string;
            }[], otherFiles: {
                unitName: string;
                content: string;
            }[], onComplete: (result: CompilerResult, program: ts.Program) => void, settingsCallback?: (settings: ts.CompilerOptions) => void, options?: ts.CompilerOptions & HarnessOptions, currentDirectory?: string): ts.CompilerOptions & HarnessOptions;
            compileDeclarationFiles(inputFiles: {
                unitName: string;
                content: string;
            }[], otherFiles: {
                unitName: string;
                content: string;
            }[], result: CompilerResult, settingsCallback?: (settings: ts.CompilerOptions) => void, options?: ts.CompilerOptions, currentDirectory?: string): {
                declInputFiles: {
                    unitName: string;
                    content: string;
                }[];
                ...
                declResult: CompilerResult;
            };
        }
        function minimalDiagnosticsToString(diagnostics: ts.Diagnostic[]): string;
        function getErrorBaseline(inputFiles: {
            unitName: string;
            content: string;
        }[], diagnostics: ts.Diagnostic[]): string;
        function collateOutputs(outputFiles: Harness.Compiler.GeneratedFile[]): string;
        ...
        */
        function getCompiler(): HarnessCompiler;
        function compileString(code: string, unitName: string, callback: (result: CompilerResult) => void): void;
        interface GeneratedFile {
            fileName: string;
            code: string;
            writeByteOrderMark: boolean;
        }
        function isTS(fileName: string): boolean;
        function isTSX(fileName: string): boolean;
        function isDTS(fileName: string): boolean;
        function isJS(fileName: string): boolean;
        function isJSX(fileName: string): boolean;
        function isJSMap(fileName: string): boolean;
        ...
        class CompilerResult {
            ...
            declFilesCode: GeneratedFile[];
            sourceMaps: GeneratedFile[];
            /** @param fileResults an array of strings for the fileName and an ITextWriter with its code */
            constructor(fileResults: GeneratedFile[], errors: ts.Diagnostic[], program: ts.Program, currentDirectoryForProgram: string, sourceMapData: ts.SourceMapData[]);
            getSourceMapRecord(): string;
        }
    }
    module TestCaseParser {
        ...
        function makeUnitsFromTest(code: string, fileName: string): {
    ...
    module Baseline {
        ...
        function runBaseline(descriptionForDescribe: string, relativeFileName: string, generateContent: () => string, runImmediately?: boolean, opts?: BaselineOptions): void;
    }
    function isLibraryFile(filePath: string): boolean;
    function isBuiltFile(filePath: string): boolean;
    function getDefaultLibraryFile(io: Harness.IO): {
        unitName: string;
        content: string;
    };
}
